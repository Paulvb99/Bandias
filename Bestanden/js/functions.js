...
function basicAjax(pURL,pElement){
	if(pElement != ''){
		new Ajax.Updater(pElement,pURL,{method:'get'});
	}else{
		new Ajax.Request(pURL,
		  {
		    ...
		    onFailure: function(){ alert("Cant complete ajax call") }
		  });
	}
}

function askConfirm(pQuestion,pUrl){
	if(confirm(pQuestion)){
		document.location.href = pUrl;
	}
}

function putMaxItems(id,value){
	if (document.getElementById(id) != null) {
