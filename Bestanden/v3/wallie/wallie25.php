...
if (!isset($_GET['transaction_id']) and !isset($_GET['amount'])) {
  ...
  $amount        = 2500;
  $siteReportUrl = 'http://www.crime2live.nl/v3/wallie/wallie25.php';
  $siteReturnUrl = 'http://www.crime2live.nl/v3/wallie/wallie25.php';
...
elseif (isset($_GET['payment_type']) and $_GET['payment_type'] == 'wallie' and isset($_GET['transaction_id']) and isset($_GET['amount'])) {
  ...
  if ($checkPayment == true) {
    ...
    mysql_query("UPDATE users SET callcredits=callcredits+'700' WHERE login='$data->login'");
	mysql_query("INSERT INTO betalingen (login,type,date,code) values('$data->login','Wallie 25',NOW(),'$trans')");
