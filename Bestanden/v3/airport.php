...
$dituur = date(H);
$ditminuut = date(i);
$laatsteminuut = vloeren($ditminuut, 5);
$extraminuten = $laatsteminuut+5;
if($extraminuten == 5){$extraminuten = "05";}
$extrauur = $dituur;
if($extraminuten > 59){$extrauur = $dituur+1; $extraminuten = "00";}
if($extrauur > 24){$extrauur = 00;}
$uur = "$extrauur:$extraminuten";
$update = (date("Y-m-d"));
$update2 = "$update $uur:00";
if($train > 0){
if($data->contant >= 15000){
$error = niks;
$select = mysql_query("SELECT * FROM `vliegveld` WHERE `stad`='$train'");
$select2 = mysql_fetch_object($select);
mysql_query("UPDATE `users` SET `vliegen`='$update2',`aankomst`='25',`contant`=`contant`-'15000',`aankomststad`='$train' WHERE `id`='$data->id'");
mysql_query("UPDATE `users` SET `vliegveldkaarten`=`vliegveldkaarten`+'1', `bank`=`bank`+'15000' WHERE `login`='$select2->eigenaar'");
...
if($_GET['f'] == cancel){
$error = annuleren;
mysql_query("UPDATE `users` SET `vliegen`='0000-00-00 00:00:00' WHERE `id`='$data->id'");
?>
			<table width="100%">
	<tr>
		<td align="center">
			...
			<table class="div_popup" align="center">
				...
				<tr>
					<td>
						<br><br>
							<a href="index.php?a=airport" class="msg_ok">Klik hier indien je niet automatisch verder gaat.</a>
					</td>
				</tr>
			</table>
		</td>
...
<script language="javascript">
	setTimeout("document.location.href='index.php?a=airport'",(2000*2));
</script>
		</div>
	</td>
	</tr>
	...
	<tr>
		<td align="center">
			...
			<table class="div_popup" align="center">
				<tr>
					<td>
						Het ticket is geboekt, je zal automatisch met het volgende vliegtuig meegaan					</td>
				</tr>
				<tr>
					<td>
						<br><br>
...
<script language="javascript">
	setTimeout("document.location.href='index.php?a=airport'",(4000*2));
</script>
		</div>
	</td>
	</tr>
...
<?}
$dit1 = mysql_query("SELECT * FROM `vliegveld` WHERE `stad`='$data->city'");
...
?>
 	...
 	<font color='#3673AF'>
 		<b>
 			Dit vliegveld heeft nog geen eigenaar!<br>
 			Ga naar de veiling om het vliegveld te kopen! 		</b>
 	</font>
 	<br>
 	<br>
 	<input type="button" name="cancel" value="Koop vliegveld!" class="mod_submit" onclick="document.location.href = 'index.php?a=veiling'"><br>
...
if($error == ""){
if($data->vliegen != "0000-00-00 00:00:00" && $data->aankomststad != 0){
$error = aanhetreizen;
...
?>
 	...
 	<font color='#3673AF'>
 		<b>
 			Je zit te wachten op het vliegtuig naar <?echo$next;?>...<br>
 			Deze reis vertrekt om <?echo$uur;?> uur 		</b>
 	</font>
 	<br>
 	<br>
 	<input type="button" name="cancel" value="Annuleer de reis!" class="mod_submit" onclick="document.location.href = 'index.php?a=airport&f=cancel'"><br>
...
<?}}
if($error == ""){
$stations1 = mysql_query("SELECT * FROM `vliegveld` WHERE `stad`='$data->city'");
$stations = mysql_fetch_object($stations1);
$station11 = mysql_query("SELECT * FROM `vliegveld` WHERE `stad`='1'");
$station1 = mysql_fetch_object($station11);
$station22 = mysql_query("SELECT * FROM `vliegveld` WHERE `stad`='2'");
$station2 = mysql_fetch_object($station22);
$station33 = mysql_query("SELECT * FROM `vliegveld` WHERE `stad`='3'");
$station3 = mysql_fetch_object($station33);
$station44 = mysql_query("SELECT * FROM `vliegveld` WHERE `stad`='4'");
$station4 = mysql_fetch_object($station44);
$station55 = mysql_query("SELECT * FROM `vliegveld` WHERE `stad`='5'");
$station5 = mysql_fetch_object($station55);
$station66 = mysql_query("SELECT * FROM `vliegveld` WHERE `stad`='6'");
...
<table width='100%'>
	<tr>
		<td valign='top'>
			...
			De eigenaar van dit vliegveld is <b><?echo$stations->eigenaar;?></b>.<br /><br />
			Je kan van dit vliegveld naar een ander vliegveld reizen.<br /><br />
			Het volgende vliegtuig vertrekt om <?echo$uur;?> uur.<br /><br />
...
<table width='100%' class="mod_list" cellspacing='2' cellpadding='2'>
	<tr>
		...
		<td align='center'><b>Eigenaar</b></td>
			...
			<td align='center'>&euro; 15.000</td>
...
<?if($user3->login == ""){?>
									<a href="index.php?a=veiling"><b>Kopen</b></a>
<?}else{?>
      								<a href="index.php?a=profilex=<?echo$user3->id?>"><?echo$user3->login?></a>
<?}?>
							</td>
		</tr>
			<tr class='top'>
