...
?>
  ...
  <div id="contentHeader"><h2>Koop credits met iDeal</h2></div>
  	...
  	<p>U heeft het volgende bedrag gekozen:</p>         
         <p>
         <b><font color="blue">iDeal 50 EURO - 1500 Call credits</font></b>
...
$description = "1500 call credits $data->login";		// Description (will appear on receipt)
$amount = 5000;							// Amount in eurocents (e.g. 1000 means € 10,00)
...
if ($step==3) {
    ...
    if (!$result) {
		...
		mysql_query("UPDATE users SET callcredits=callcredits+'1500' WHERE login='$data->login'");
		mysql_query("INSERT INTO betalingen (login,type,date) values('$data->login','iDeal 50',NOW())");
